...
impl EncryptedStore {
    ...
    pub fn upsert_workspace(&self, workspace: &Workspace) -> Result<(), StorageError> {
        ...
        connection.execute(
            "INSERT INTO workspace_enrollments
             (id, root, repository_identity, excluded_paths, revoked)
             VALUES (?1, ?2, ?3, ?4, ?5)
    ...
    pub fn representative_request_result(
        ...
        let row = connection
            .query_row(
                "SELECT consumed_at, completed_at, result_json
                 FROM representative_requests
                 WHERE request_id = ?1",
                [request_id],
                |row| {
            ...
            .optional()?;
        let Some((consumed_at, completed_at, result_json)) = row else {
            return Ok(None);
        };
    ...
    pub fn resolve_integration_workstream(
        ...
        connection.execute(
            "INSERT INTO integration_sessions
             (session_key, workspace_id, workstream_id, created_at)
