...
export async function runSidecar(
  ...
  if (health.status !== "ok" || health.protocolVersion !== 1) {
    throw new Error("interod is incompatible with this Local Stand-in.");
  }
    ...
    if (Date.now() >= nextHeartbeatAt) {
      try {
        ...
        process.stderr.write(
          `${JSON.stringify({
            level: "warn",
            operation: "stand_in.settings_refresh",
...
export async function processQueuedRequest(
  ...
  await daemon.call("stand_in.complete_request", {
    requestId: request.requestId,
    result,
  });
}
...
function requestToEvent(
  ...
  if (request.method === "stand_in.ingest_adapter_event") {
  ...
  if (request.method !== "stand_in.report_checkpoint") return undefined;
...
async function requestCoordination(
  ...
  const standInId = localStandInId();
...
async function requestSpecReview(
  ...
  const created = await apiRequest<{
    ...
    body: {
      ...
      markdown,
      changeSummary:
        "Review requested by a Coding Agent through its Stand-in.",
      affectedScopes,
...
async function apiRequest<T = unknown>(
  ...
  if (!apiUrl)
    throw new Error(
      "INTERO_API_URL is required for this Stand-in request.",
    );
...
function localStandInId(): PrincipalId {
  return (process.env.INTERO_LOCAL_STAND_IN_ID ??
