...
Calling needs `INTERO_LIVEKIT_API_SECRET`; the bundled LiveKit API key is
fixed to `intero`. The signaling URL is derived from `INTERO_PUBLIC_URL` as
`wss://<host>/rtc`. `pnpm production:deploy` refuses to start while any key
from `.env.production.example` is missing from `.env.production`.
